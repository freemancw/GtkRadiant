...
typedef struct face_s
{
	...
	// it would be good if the rendering loop would avoid scanning there 
    // (for the GL binding number for example)
	IShader                 *pShader;
	...
	// NOTE: casting can be done with a GETPLUGINTEXDEF defined in 
    //       isurfaceplugin.h
	// TODO: if the __ISURFACEPLUGIN_H_ header is used, use a union 
    //       { void *pData; IPluginTexdef *pPluginTexdef } kind of thing ?
	void                    *pData;
} face_t;
